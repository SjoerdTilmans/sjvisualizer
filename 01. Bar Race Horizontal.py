"""Bar Race Horizontal with generated data.

Run: python "01. Bar Race Horizontal.py" --seconds 12 --fps 30
...
def build_chart(cv, df):
    ...
    return BarRace.bar_race(
        canvas=cv, df=df, x_pos=100, y_pos=150,
        width=900, height=450, font_size=18,
        orientation="horizontal", number_of_bars=3, allow_decrease=True,
        unit=" GWh",
    )
...
def main():
    ...
    cv.tk.title("SJVisualizer - Bar Race Horizontal")
    if args.smoke:
        cv.tk.withdraw()
    try:
        cv.add_title("Bar Race Horizontal")
        ...
        if args.smoke:
            ...
            print("Bar Race Horizontal: render smoke passed")
        else:
