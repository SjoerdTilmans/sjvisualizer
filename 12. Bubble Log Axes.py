"""Bubble Log Axes with generated data.

Run: python "12. Bubble Log Axes.py" --seconds 12 --fps 30
...
def build_chart(cv, df, args=None):
    ...
    # Log coordinates must be positive; use several orders of magnitude.
    x = pd.DataFrame({name: 10 ** (2 + 1.5 * np.sin(t + i))
                      for i, name in enumerate(df.columns)}, index=df.index)
    y = pd.DataFrame({name: 10 ** (2 + 1.5 * np.cos(t + i * .7))
                      for i, name in enumerate(df.columns)}, index=df.index)
...
def main():
    ...
    cv.tk.title("SJVisualizer - Bubble Log Axes")
    if args.smoke:
        cv.tk.withdraw()
    try:
        cv.add_title("Bubble Log Axes")
        ...
        if args.smoke:
            ...
            print("Bubble Log Axes: render smoke passed")
        else:
