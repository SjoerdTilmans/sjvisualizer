"""Line Numeric XY with generated data.

Run: python "06. Line Numeric XY.py" --seconds 12 --fps 30
...
def build_chart(cv, df):
    ...
    # Numeric X values are independent of the animation dates.
    t = np.linspace(0, 1, len(df))
    x = pd.DataFrame({"Solar": t ** 2, "Wind": t, "Hydro": np.sqrt(t)},
                     index=df.index)
    return LineChart.line_chart(
        canvas=cv, df=df, x_pos=100, y_pos=150,
        width=900, height=450, font_size=18,
        x_df=x, x_min=0, x_max=1, x_decimal_places=2, draw_points=True,
    )
...
def main():
    ...
    cv.tk.title("SJVisualizer - Line Numeric XY")
    if args.smoke:
        cv.tk.withdraw()
    try:
        cv.add_title("Line Numeric XY")
        ...
        if args.smoke:
            ...
            print("Line Numeric XY: render smoke passed")
        else:
