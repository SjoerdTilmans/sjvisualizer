...
class dynamic_curve(cv.sub_plot):
    ...
    def draw(self, time):
        ...
        # create line object
        self.line = self.canvas.create_line(0,0,0,0, fill=cv._from_rgb(self.color), width=int(self.marker_size/3))

        # loop over all values in the row
        spacing = self.width / (len(data))
        x_pos = self.x_pos + spacing / 2
        for i, (name, d) in enumerate(data.items()):
            self.graph_elements[name] = graph_element(name=name, pos=x_pos, canvas=self.canvas, value=d, unit=self.unit, font_color=self.font_color, colors=self.colors, chart=self, text_font=self.text_font, font_size=self.font_size)
            x_pos = x_pos + spacing
...
class graph_element():
    ...
    def __init__(self, name=None, pos=0, canvas=None, value=0, unit=None, font_color=(0,0,0), colors={}, font_size=12, chart=None, text_font="Microsoft JhengHei UI"):
        self.name = name
        self.canvas = canvas
        self.unite = unit
    ...
    def update(self, value):
        ...
        y = self.chart.y_pos+0.85*self.chart.height-self.chart.axis.calc_positions(value)
        x = self.pos
...
if __name__ == "__main__":
    ...
    dc = dynamic_curve(canvas=canvas, df=df, font_size=25, font_color=(0, 0, 0))
