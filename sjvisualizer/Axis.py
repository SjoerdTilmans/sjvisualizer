...
class axis:
    ...
    - draw() and update() accept legacy keyword args: min=..., max=...
      but internally we use min_val/max_val to avoid shadowing built-ins.
    """
    ...
    def __init__(
        ...
        anchor="s",
        decimal_places=0,
        zero_based=False,     # optional: keep axis floor at 0 when incoming min is >= 0
        sticky_min=None,      # optional: override sticky behavior per side
        sticky_max=None,      # optional: override sticky behavior per side
    ):
        self.canvas = canvas
        self.x = x
        self.y = y
        ...
        # Sticky defaults: original behavior when allow_decrease=False => range only expands
    ...
    def _coerce_legacy_minmax_kwargs(self, min_val, max_val, kwargs):
        # Backwards compatibility: allow update(min=..., max=...) and draw(min=..., max=...)
        if "min" in kwargs:
            min_val = kwargs["min"]
        if "max" in kwargs:
            max_val = kwargs["max"]
    ...
    def _update_limits(self, incoming_min, incoming_max):
        ...
        if self.min_val is None or self.max_val is None:
            ...
            # Sticky behavior: only expand range unless allow_decrease=True (or sticky_* overridden)
        ...
        # Optional: if incoming data is non-negative, clamp axis minimum to 0
    ...
    def update(self, min_val=0, max_val=0, **kwargs):
        ...
        if not self.is_date:
            ...
            # Ensure we have enough tick objects
            ...
            # FIX: don't hide the last valid tick; start at last_used + 1
...
class tick:
    ...
    def draw(self, value=0):
        self.line = self.canvas.create_line(
            -1, -1, -1, -1,
            fill=cv._from_rgb(self.axis.color),
        ...
        if draw:
            ...
            if self.axis.orientation == "horizontal":
                if self.label_pos == "s":
                    self.canvas.coords(
                        self.line,
                        self.axis.x + pos, self.axis.y - self.length - l,
                        self.axis.x + pos, self.axis.y + 10
                    )
                elif self.label_pos == "n":
                    self.canvas.coords(
                        self.line,
                        self.axis.x + pos, self.axis.y,
                        self.axis.x + pos, self.axis.y + 10 + l
                    )
                else:
                    self.canvas.coords(
                        self.line,
                ...
                if l:
                    self.canvas.coords(
                        self.line,
                        self.axis.x - 10, self.axis.y - pos,
                        self.axis.x + self.length + l, self.axis.y - pos
                    )
                else:
                    self.canvas.coords(
                        self.line,
                        self.axis.x - 10, self.axis.y - pos,
                        self.axis.x + self.length, self.axis.y - pos
                    )
...
# Helpers to avoid shadowing issues and keep intent explicit
