"""Dynamic Matrix with generated data.

Run: python "14. Dynamic Matrix.py" --seconds 12 --fps 30
...
from sjvisualizer import Canvas, DynamicMatrix
...
def build_chart(cv, df):
    ...
    df = (df - 45) / 20
    return DynamicMatrix.dynamic_matrix(
        canvas=cv, df=df, x_pos=100, y_pos=150,
        width=900, height=450, font_size=18,
        level_count=3, neutral_string="Neutral",
    )
...
def main():
    ...
    cv.tk.title("SJVisualizer - Dynamic Matrix")
    if args.smoke:
        cv.tk.withdraw()
    try:
        cv.add_title("Dynamic Matrix")
        ...
        if args.smoke:
            ...
            print("Dynamic Matrix: render smoke passed")
        else:
