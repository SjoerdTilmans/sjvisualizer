"""USA States Map with generated data.

Run: python "17. USA States Map.py" --seconds 12 --fps 30
...
from sjvisualizer import Canvas, Map
...
def build_chart(cv, df):
    ...
    # Column names must match the packaged map regions.
    names = ['California', 'Texas', 'New York', 'Alaska', 'Hawaii']
    df = pd.DataFrame({name: df.iloc[:, i % len(df.columns)] * (1 + .15 * i)
                       for i, name in enumerate(names)}, index=df.index)
    return Map.map(
...
def main():
    ...
    cv.tk.title("SJVisualizer - USA States Map")
    if args.smoke:
        cv.tk.withdraw()
    try:
        cv.add_title("USA States Map")
        ...
        if args.smoke:
            ...
            print("USA States Map: render smoke passed")
        else:
