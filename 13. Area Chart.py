"""Area Chart with generated data.

Run: python "13. Area Chart.py" --seconds 12 --fps 30
...
from sjvisualizer import Canvas, AreaChart, DataHandler
...
def build_chart(cv, df, args=None):
    ...
    return AreaChart.area_chart(
        canvas=cv, df=df, x_pos=100, y_pos=150,
        width=900, height=450, font_size=18,
        display_values=True, label_position="right",
        events={} if args and args.excel else {"Example event": ["01/01/2010", "01/01/2013"]},
    )
...
def main():
    ...
    cv.tk.title("SJVisualizer - Area Chart")
    if args.smoke:
        cv.tk.withdraw()
    try:
        cv.add_title("Area Chart")
        ...
        if args.smoke:
            ...
            print("Area Chart: render smoke passed")
        else:
