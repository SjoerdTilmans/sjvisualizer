"""Stacked Bar Chart with generated data.

Run: python "03. Stacked Bar Chart.py" --seconds 12 --fps 30
...
from sjvisualizer import Canvas, StackedBarChart
...
def build_chart(cv, df):
    ...
    return StackedBarChart.stacked_bar_chart(
...
def main():
    ...
    cv.tk.title("SJVisualizer - Stacked Bar Chart")
    if args.smoke:
        cv.tk.withdraw()
    try:
        cv.add_title("Stacked Bar Chart")
        ...
        if args.smoke:
            ...
            print("Stacked Bar Chart: render smoke passed")
        else:
            cv.play(df=df, fps=args.fps)
            # Hold the final timestamp while the newest bar settles.
            cv.play(df=pd.DataFrame(index=[df.index[-1]] * chart.animation_frames),
                    fps=args.fps)
            cv.tk.mainloop()
    finally:
        try:
