...
def main(fps = 60, duration = 0.2):
    ...
    # add bar chart
    bar_chart = BarRace.bar_race(canvas=canvas, df=df, title="Bar race", colors=colors, height=chart_height,
                                 width=int(width / 6), x_pos=int(height / 3 / 2), y_pos=int(width / 5) / 2, back_ground_color=(255,0,0))
    ...
    stacked = StackedBarChart.stacked_bar_chart(canvas=canvas, df=df, title="Stacked", colors=colors,
                                                height=chart_height,
    ...
    # add pie chart
    pie_plot = PieRace.pie_plot(canvas=canvas, df=df, title="Pie race", colors=colors, height=chart_height,
    ...
    # add time indication
    date = Date.date(canvas=canvas, height=int(height / 20),
    ...
    img = StaticImage.static_image(canvas=canvas, file="assets/Made with SJvisualzer.png", width=height/20, height=height/20, x_pos=width/3*2.25, y_pos=height/1.4)
