"""Line Log Y with generated data.

Run: python "07. Line Log Y.py" --seconds 12 --fps 30
...
def build_chart(cv, df):
    ...
    return LineChart.line_chart(
        ...
        y_log=True, y_min=1, y_max=10000, draw_points=False,
    )
...
def main():
    ...
    cv.tk.title("SJVisualizer - Line Log Y")
    if args.smoke:
        cv.tk.withdraw()
    try:
        cv.add_title("Line Log Y")
        ...
        if args.smoke:
            ...
            print("Line Log Y: render smoke passed")
        else:
